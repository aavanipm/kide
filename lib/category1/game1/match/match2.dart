...
class Match2 extends StatefulWidget {
  ...
  const Match2({Key? key, required this.username, required this.email, required this.age, required this.subscribedCategory}) : super(key: key);
...
class _Match2State extends State<Match2> {
  ...
  List<List<Animal>> animalSets = [
    [
      Animal(name: 'potato', imagePath: 'assets/Vegetables/potato.png'),
      Animal(name: 'pig', imagePath: 'assets/animals/pig.png'),
      Animal(name: 'lemon', imagePath: 'assets/Fruits/lemon.png'),
      Animal(name: 'hen', imagePath: 'assets/Birds/hen.png'),
    ],
  ];
  ...
  void _checkAnswers() {
    ...
    if (correctAnswers == totalCorrectAnswers && !gameCompleted) {
      if (score == 4){
        score = 8;// Set score to 3 directly
  ...
  void _proceedToNextLevel() {
    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>Match3(
      username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory,
    )));
  }
