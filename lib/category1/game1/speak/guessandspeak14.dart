...
class GuessandSpeak14 extends StatefulWidget {
  ...
  const GuessandSpeak14({Key? key, required this.username, required this.email,
    required this.age, required this.subscribedCategory}) : super(key: key);

  @override
  _GuessandSpeak14State createState() => _GuessandSpeak14State();
}

class _GuessandSpeak14State extends State<GuessandSpeak14> {
  final List<AnimalItem> animals = [
    AnimalItem(name: 'tulip', imagePath: 'assets/Flowers/tulip.png'),
  ];
      ...
      Future.delayed(Duration(seconds: 3), () {
        if (widget.subscribedCategory == 'premium') {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GuessandSpeak15(
                username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory,
              ),
            ),
          );
