...
class GuessandSpeak13 extends StatefulWidget {
  ...
  const GuessandSpeak13({Key? key, required this.username, required this.email,
    required this.age, required this.subscribedCategory}) : super(key: key);

  @override
  _GuessandSpeak13State createState() => _GuessandSpeak13State();
}

class _GuessandSpeak13State extends State<GuessandSpeak13> {
  final List<AnimalItem> animals = [
    AnimalItem(name: 'goat', imagePath: 'assets/animals/goat.png'),
  ];
      ...
      Future.delayed(Duration(seconds: 3), () {
        if (widget.subscribedCategory == 'premium' ||
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GuessandSpeak14(
                username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory,
              ),
            ),
          );
