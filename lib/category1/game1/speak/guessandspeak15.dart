...
class GuessandSpeak15 extends StatefulWidget {
  final String username;
  final String email;
  final String age;
  final String subscribedCategory;

  const GuessandSpeak15(
      {Key? key,
  ...
  @override
  _GuessandSpeak15State createState() => _GuessandSpeak15State();
}

class _GuessandSpeak15State extends State<GuessandSpeak15> {
  ...
  void _checkAnswer(String spokenPhrase) {
    String correctPhrase = "Hello there".toLowerCase(); // Concatenate "My name is" with the username
      ...
      Future.delayed(Duration(seconds: 3), () {
        if (widget.subscribedCategory == 'premium') {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GuessandSpeak16(
                username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory,
              ),
            ),
          );
