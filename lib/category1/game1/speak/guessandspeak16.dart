...
class GuessandSpeak16 extends StatefulWidget {
  final String username;
  final String email;
  final String age;
  final String subscribedCategory;

  const GuessandSpeak16(
      {Key? key,
  ...
  @override
  _GuessandSpeak16State createState() => _GuessandSpeak16State();
}

class _GuessandSpeak16State extends State<GuessandSpeak16> {
    ...
    return Scaffold(
      ...
      body: Column( // Wrap with Stack instead of Column
        children: [
          ...
          SizedBox(height: 150,),
          Center(
            child: Text("THE SKY IS BLUE", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 30),),
          ),
          SizedBox(height: 20,),
          IconButton(
  ...
  void _checkAnswer(String spokenPhrase) {
    String correctPhrase = "THE SKY IS BLUE".toLowerCase(); // Concatenate "My name is" with the username
      ...
      Future.delayed(Duration(seconds: 3), () {
        if (widget.subscribedCategory == 'premium') {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GuessandSpeak17(
                username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory,
              ),
            ),
          );
