...
class GuessandSpeak18 extends StatefulWidget {
  final String username;
  final String email;
  final String age;
  final String subscribedCategory;

  const GuessandSpeak18(
      {Key? key,
  ...
  @override
  _GuessandSpeak18State createState() => _GuessandSpeak18State();
}

class _GuessandSpeak18State extends State<GuessandSpeak18> {
    ...
    return Scaffold(
      ...
      body: Column( // Wrap with Stack instead of Column
        children: [
          ...
          SizedBox(height: 150,),
          Center(
            child: Text("The dog barks loudly", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 30),),
          ),
          SizedBox(height: 20,),
          IconButton(
  ...
  void _checkAnswer(String spokenPhrase) {
    String correctPhrase = "The dog barks loudly".toLowerCase(); // Concatenate "My name is" with the username
    ...
    if (spokenPhrase.startsWith(correctPhrase)) { // Check if the spoken phrase starts with the correct phrase
      ...
      Future.delayed(Duration(seconds: 3), () {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => GuessandSpeak19(
              username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory
          )),
        );
      });
