...
class GuessandSpeak19 extends StatefulWidget {
  final String username;
  final String email;
  final String age;
  final String subscribedCategory;

  const GuessandSpeak19(
      {Key? key,
  ...
  @override
  _GuessandSpeak19State createState() => _GuessandSpeak19State();
}

class _GuessandSpeak19State extends State<GuessandSpeak19> {
    ...
    return Scaffold(
      ...
      body: Column( // Wrap with Stack instead of Column
        children: [
          ...
          SizedBox(height: 150,),
          Center(
            child: Text("The baby is smiling", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 30),),
          ),
          SizedBox(height: 20,),
          IconButton(
  ...
  void _checkAnswer(String spokenPhrase) {
    String correctPhrase = "The baby is smiling".toLowerCase(); // Concatenate "My name is" with the username
    ...
    if (spokenPhrase.startsWith(correctPhrase)) { // Check if the spoken phrase starts with the correct phrase
      ...
      Future.delayed(Duration(seconds: 3), () {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => GuessandSpeak20(
              username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory
          )),
        );
      });
