...
class GuessandSpeak2 extends StatefulWidget {

  final String username;
  final String email;
  final String age;
  const GuessandSpeak2({Key? key, required this.username, required this.email, required this.age}) : super(key: key);

  @override
  _GuessandSpeak2State createState() => _GuessandSpeak2State();
}

class _GuessandSpeak2State extends State<GuessandSpeak2> {
  final List<AnimalItem> animals = [
    AnimalItem(name: 'apple', imagePath: 'assets/Fruits/apple.png'),
  ];
  ...
  void _checkAnswer(String guessedAnimal) {
    ...
    if (spokenWordLower == correctAnimalName || _isSimilar(spokenWordLower, correctAnimalName)) {
      ...
      Future.delayed(Duration(seconds: 3), () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => GuessandSpeak3(
                username: widget.username, email: widget.email, age: widget.age
            )));
      });
    } else {
