...
class GuessandSpeak1 extends StatefulWidget {
  ...
  const GuessandSpeak1({Key? key, required this.username, required this.email,
    required this.age, required this.subscribedCategory}) : super(key: key);

  @override
  _GuessandSpeak1State createState() => _GuessandSpeak1State();
}

class _GuessandSpeak1State extends State<GuessandSpeak1> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          SizedBox(height: 30,),
          Text(
            "Press the mic button...and say", style: TextStyle(fontSize: 20),),
          Text("Who is this?", style: TextStyle(fontSize: 25),),
          SizedBox(height: 150,),
          Center(
            child: _buildAnimalCard(
                animals[0]), // Assuming you want the first animal in the list
          ),
          IconButton(
  ...
  void _checkAnswer(String guessedAnimal) {
    ...
    if (spokenWordLower == correctAnimalName ||
        _isSimilar(spokenWordLower, correctAnimalName)) {
      setState(() {
        if (score == 0) {
          score = 1;
  ...
  void _showSnackbar(String message, bool isCorrect) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isCorrect ? Colors.green : Colors
            .red, // Change color based on correctness
      ),
    );
  }
    ...
    _speech.listen(
      onResult: (result) {
        if (result.finalResult) {
          ...
          _checkAnswer(
              result.recognizedWords); // Move this line to _checkAnswer method
        }
      },
    );
  }
