...
class GuessandSpeak6 extends StatefulWidget {
  ...
  const GuessandSpeak6({Key? key, required this.username, required this.email,
...
class _GuessandSpeak6State extends State<GuessandSpeak6> {
  final List<AnimalItem> animals = [
    AnimalItem(name: 'strawberry', imagePath: 'assets/Fruits/strawberry.png'),
  ];
  ...
  void _checkAnswer(String guessedAnimal) {
    ...
    if (spokenWordLower == correctAnimalName || _isSimilar(spokenWordLower, correctAnimalName)) {
      ...
      // Automatically navigate to level 2 after 3 seconds
      Future.delayed(Duration(seconds: 3), () {
        if (widget.subscribedCategory == 'basic' ||
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (context) => GuessandSpeak7(
                username: widget.username, email: widget.email, age: widget.age, subscribedCategory: widget.subscribedCategory,
              ),
            ),
          );
