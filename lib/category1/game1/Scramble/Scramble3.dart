...
class _Scramble3State extends State<Scramble3> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Scramble Fruits'),
  ...
  bool _isValidWord(String word) {
    List<String> validWords = [
      'APPLE', 'BANANA', 'ORANGE', 'GRAPE', 'STRAWBERRY', 'WATERMELON', 'KIWI', 'MANGO',
      'PINEAPPLE', 'PEACH', 'PEAR', 'PLUM', 'CHERRY', 'BLUEBERRY', 'RASPBERRY', 'LEMON',
      'LIME', 'POMEGRANATE', 'APRICOT', 'AVOCADO', 'COCONUT', 'FIG', 'GUAVA', 'PAPAYA',
      'NECTARINE', 'CANTALOUPE', 'HONEYDEW', 'STARFRUIT', 'PASSIONFRUIT', 'LYCHEE',
      'DRAGONFRUIT', 'JACKFRUIT', 'TANGERINE', 'KIWIFRUIT', 'CLEMENTINE', 'MANDARIN',
      'BLACKBERRY', 'CRANBERRY', 'DATE',
    ];
    ...
    if (snapshot.exists) {
      ...
      if (data.containsKey('fruit')) {
        setState(() {
          ...
          foundWords = List<String>.from(data['fruit']['foundWords']);
          playedWords = Set<String>.from(data['fruit']['foundWords']); // Update playedWords
        });
      }
    }
  }
