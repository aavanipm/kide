...
class GuessingGame14 extends StatefulWidget {
  final String username;
  final String email;
  final String age;
  final String subscribedCategory;

  GuessingGame14({Key? key, required this.username, required this.email, required this.age, required this.subscribedCategory}) : super(key: key);
  @override
  _GuessingGame14State createState() => _GuessingGame14State();
}

class _GuessingGame14State extends State<GuessingGame14> {
  int score = 0;
  List<Animal> animals = [
    Animal(name: 'flamingo', imagePath: 'assets/Birds/flamingo.png', points: 1),
    Animal(name: 'cow', imagePath: 'assets/animals/cow.png', points: 1),
    Animal(name: 'swan', imagePath: 'assets/Birds/swan.png', points: 1),
                ...
                ElevatedButton(
                  onPressed: () {
                    if (widget.subscribedCategory == 'premium' || widget.subscribedCategory == 'standard') {
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => GuessingGame15(
  ...
  void _submitGuesses() {
    ...
    for (var animal in animals) {
      if (animal.matched == false && animal.name.toLowerCase() == animal.typedName.trim().toLowerCase()) {
        ...
        if(score==32 || score==33 || score==34 || score==35){
