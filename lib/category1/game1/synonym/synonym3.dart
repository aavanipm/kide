...
class Synonym3 extends StatefulWidget {
  ...
  const Synonym3({Key? key, required this.username, required this.email, required this.age, required this.subscribedCategory}) : super(key: key);
  ...
  void _checkAnswer(String selectedWord) {
    setState(() {
      if (wordSynonyms[currentWord]!.contains(selectedWord)) {
        if (score == 6) {
          score = 9; // Set score to 3 directly
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
                onPressed: () {
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Synonym4(
