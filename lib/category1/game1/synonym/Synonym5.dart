...
class Synonym5 extends StatefulWidget {
  final String username;
  final String email;
  final String age;
  final String subscribedCategory;

  const Synonym5({Key? key, required this.username, required this.email, required this.age, required this.subscribedCategory}) : super(key: key);

  @override
  _Synonym5State createState() => _Synonym5State();
}

class _Synonym5State extends State<Synonym5> {
  final Map<String, List<String>> wordSynonyms = {
    'fast': ['quick', 'rapid', 'speedy'],
  };

  final Map<String, List<String>> wordNotSimilar = {
    'fast': ['slow', 'gradual', 'relaxed', 'moderate'],
  };
  ...
  void _checkAnswer(String selectedWord) {
    setState(() {
      if (wordSynonyms[currentWord]!.contains(selectedWord)) {
        if (score == 12) {
          score = 15; // Set score to 3 directly
