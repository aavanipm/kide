...
class Synonym4 extends StatefulWidget {
  ...
  const Synonym4({Key? key, required this.username, required this.email, required this.age, required this.subscribedCategory}) : super(key: key);
...
class _Synonym4State extends State<Synonym4> {
  ...
  final Map<String, List<String>> wordNotSimilar = {
    'small': ['large', 'huge', 'massive', 'giant'],
  ...
  void _checkAnswer(String selectedWord) {
    setState(() {
      if (wordSynonyms[currentWord]!.contains(selectedWord)) {
        if (score == 9) {
          score = 12; // Set score to 3 directly
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
                onPressed: () {
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Synonym5(
