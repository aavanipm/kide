...
import 'package:game/category1/game1/synonym/synlast.dart';
...
class Synonym8 extends StatefulWidget {
  ...
  const Synonym8({Key? key, required this.username, required this.email, required this.age, required this.subscribedCategory}) : super(key: key);
...
class _Synonym8State extends State<Synonym8> {
  final Map<String, List<String>> wordSynonyms = {
    'bad': ['poor', 'awful', 'unpleasant']
  };

  final Map<String, List<String>> wordNotSimilar = {
    'bad': ['excellent', 'great', 'fantastic', 'superb',]
  };
  ...
  void _checkAnswer(String selectedWord) {
    setState(() {
      if (wordSynonyms[currentWord]!.contains(selectedWord)) {
        if (score == 21) {
          score = 24; // Set score to 3 directly
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
                onPressed: () {
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => SynLast(
